...
#ifndef _DIJON_CHMFILTER_H
#define _DIJON_CHMFILTER_H
...
namespace Dijon
{
    class ChmFilter : public Filter
    {
    public:
	/// Builds an empty filter.
	ChmFilter(const std::string &mime_type);
	/// Destroys the filter.
	virtual ~ChmFilter();
	...
	/// Adds a unit.
	void add_unit(struct chmUnitInfo *pUnitInfo);


    protected:
	size_t m_maxSize;
	struct chmFile *m_pHandle;
	std::vector<struct chmUnitInfo*> m_units;
	bool m_doneAll;
    ...
    private:
	/// ChmFilter objects cannot be copied.
	ChmFilter(const ChmFilter &other);
	/// ChmFilter objects cannot be copied.
	ChmFilter& operator=(const ChmFilter& other);

    };
}

#endif // _DIJON_CHMFILTER_H
