...
#ifndef _DIJON_JSONFILTER_H
#define _DIJON_JSONFILTER_H
...
namespace Dijon
{
    class JsonFilter : public Filter
    {
    public:
	...
	void dump_object(const std::string &key,
		json_object *pObj);

	bool parse_json(const std::string &json_doc);

    private:
	/// JsonFilter objects cannot be copied.
	JsonFilter(const JsonFilter &other);
	/// JsonFilter objects cannot be copied.
	JsonFilter& operator=(const JsonFilter& other);

    };
}

#endif // _DIJON_JSONFILTER_H
