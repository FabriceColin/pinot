...
#ifndef _DIJON_EXIV2IMAGEFILTER_H
#define _DIJON_EXIV2IMAGEFILTER_H
...
namespace Dijon
{
    class Exiv2ImageFilter : public Filter
    {
    public:
	/// Builds an empty filter.
	Exiv2ImageFilter(const std::string &mime_type);
	/// Destroys the filter.
	virtual ~Exiv2ImageFilter();
    ...
    private:
	/// Exiv2ImageFilter objects cannot be copied.
	Exiv2ImageFilter(const Exiv2ImageFilter &other);
	/// Exiv2ImageFilter objects cannot be copied.
	Exiv2ImageFilter& operator=(const Exiv2ImageFilter& other);

    };
}

#endif // _DIJON_EXIV2IMAGEFILTER_H
